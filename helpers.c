...
{
    // Dynamically allocate memory for the copy on the heap
    RGBTRIPLE(*copy)[width] = malloc(height * width * sizeof(RGBTRIPLE));
    
    // Safety check in case system is out of memory
    if (copy == NULL)
    {
        return;
    }
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            // Count the number of surrounding grids within image
            int cnt = 0;
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            // Cap values at 255 and change the image pixel values
